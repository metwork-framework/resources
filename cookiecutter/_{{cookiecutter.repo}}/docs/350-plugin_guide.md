...
{{ nested_raw() }}
```
{{ "{{" + cookiecutter.repo|upper + "_HOME}}" }}/bin/cronwrap.sh --lock --log-capture-to your_command.log -- plugin_wrapper {{ "{{MFSERV_CURRENT_PLUGIN_NAME}}" }} --
```

It will:

- replace automatically `{{ "{{" + cookiecutter.repo|upper + "_HOME}}" }} }}`, `{{ "{{" + cookiecutter.repo|upper + "_CURRENT_PLUGIN_NAME}}" }}`, `{{ "{{" + cookiecutter.repo|upper + "_CURRENT_PLUGIN_DIR + "}}" }}`, `{{ "{{" + cookiecutter.repo|upper + "_CURRENT_PLUGIN_CUSTOM_*}}" }}` variables (to avoid hardcoding things) ([full list of available variables](../850-reference/700-env/)
...
??? question "How to debug?"
    ...
    for all plugins and `{{cookiecutter.repo}}` module.
