...
```
{% raw %}{{{% endraw %}{{cookiecutter.repo|upper}}{% raw %}_HOME}}{% endraw %}/bin/cronwrap.sh --lock --log-capture-to your_command.log -- plugin_wrapper {% raw %}{{MFSERV_CURRENT_PLUGIN_NAME}}{% endraw %} --
```

It will:

- replace automatically `{% raw %}{{{% endraw %}{{cookiecutter.repo|upper}}{% raw %}_HOME}}{% endraw %}`, `{% raw %}{{MFSERV_CURRENT_PLUGIN_NAME}}{% endraw %}` and `{% raw %}{{MFSERV_CURRENT_PLUGIN_DIR}}{% raw %}` variables (to avoid hardcoding things)
