...
| `crontab` | plugin crontab (use something like `* * * * * {% raw %}{{{% endraw %}{{cookiecutter.repo|upper}}_HOME{% raw %}}}{% endraw %}/bin/cronwrap.sh -l -e --log-capture-to your_command.log -- plugin_wrapper {YOUR_PLUGIN_NAME} {YOUR_COMMAND}` for each line to execute your command in your plugin environement) |
