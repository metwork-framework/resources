...
| `crontab` | plugin crontab (use something like `* * * * * {% raw %}{{{% endraw %}{{cookiecutter.repo|upper}}_HOME{% raw %}}}{% endraw %}/bin/cronwrap.sh -l -e -- plugin_wrapper {YOUR_PLUGIN_NAME} {YOUR_COMMAND} >>{{MODULE_RUNTIME_HOME}}/log/your_command.log 2>&1` for each line to execute your command in your plugin environement) |
